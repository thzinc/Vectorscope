...
def menu_custom(the_menu):
    ...
    if the_menu.level == 1:
        if machine.Pin(22).value():
            the_menu.current[sound_index] = [" Sound Off", toggle_sound, 0]
        else:
            the_menu.current[sound_index] = [" Sound On", toggle_sound, 1]
...
async def vos_main():
    ...
    while True:  # since this is the main menu, we don't really ever quit
